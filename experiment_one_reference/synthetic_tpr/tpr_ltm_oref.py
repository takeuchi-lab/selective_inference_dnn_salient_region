    ...
    while flag:
        try:
            ...
            if method["parametric"]:
                p_value = cam_si_thr.inference(X, ref=ref,thr=thr)[0]
                result_dic["parametric"] = p_value
            
            if method["oc"]:
                p_value = cam_si_thr.inference(X, ref=ref,oc=True,thr=thr)[0]
                result_dic["oc"] = p_value
            
            if method["naive"]:
                p_value =  util.naive_p_mean(X,ref,output,1,thr=thr)
                result_dic["naive"] = p_value
            
            if method["bonf"]:
                p_value = util.naive_p_mean_bonf(X, ref, output,1,thr=thr)
                result_dic["bonf"] = p_value
            
            if method["permutation1"]:
                p_value = util.permutation_test_mean(X, ref, output, model_with_cam,thr=thr,test=test)
                result_dic["permutation1"] = p_value
            
            if method["permutation2"]:
                p_value =  util.permutation_test_mean_2(X, ref, output, model_with_cam,thr=thr,test=test)
...
if __name__ == "__main__":
    ...
    result_path = "./experiment_result/tpr_thr_ltm_oref"

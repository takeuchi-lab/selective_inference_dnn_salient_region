...
def run(args):
    ...
    while flag:
        try:
            ...
            if method["naive"]:
                p_value =  util.naive_p_abs(X,ref,output,1)
                result_dic["naive"] = p_value
            
            if method["bonf"]:
                p_value = util.naive_p_abs_bonf(X, ref, output,1)
                result_dic["bonf"] = p_value
            
            if method["permutation1"]:
                p_value = util.permutation_test_abs(X, ref, output, model_with_cam,test=test)
                result_dic["permutation1"] = p_value
            
            if method["permutation2"]:
                p_value =  util.permutation_test_abs_2(X, ref, output, model_with_cam,test=test)
...
if __name__ == "__main__":
    ...
    if is_tracking:
        ...
        EXPERIMENT_NAME = "final_fpr_lta_oref"
    ...
    result_path = "./experiment_result/fpr_thr_lto_mref"

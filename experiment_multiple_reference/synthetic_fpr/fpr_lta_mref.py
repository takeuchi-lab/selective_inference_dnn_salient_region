...
def run(args):
    ...
    cam_si_thr = cam_si.si4dnn_cam_si_thr_abs(model_with_cam)
    ...
    while flag:
        try:
            ...
            if method["naive"]:
                p_value =  util.naive_p_abs(X,ref,output,1,multi_ref=True)
                result_dic["naive"] = p_value
            
            if method["bonf"]:
                p_value = util.naive_p_abs_bonf(X, ref, output,1,multi_ref=True)
                result_dic["bonf"] = p_value
            
            if method["permutation1"]:
                p_value = util.permutation_test_abs(X, ref, output, model_with_cam,multi_ref=True,test=test)
                result_dic["permutation1"] = p_value
            
            if method["permutation2"]:
                p_value =  util.permutation_test_abs_2(X, ref, output, model_with_cam,multi_ref=True,test=test)
...
if __name__ == "__main__":
    ...
    if is_tracking:
        ...
        EXPERIMENT_NAME = "final_fpr_lta_2ref"
    ...
    result_path = "./experiment_result/fpr_thr_lta_mref"

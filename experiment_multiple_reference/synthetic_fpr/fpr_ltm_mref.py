...
def run(args):
    ...
    while flag:
        try:
            ...
            if method["naive"]:
                p_value =  util.naive_p_mean(X,ref,output,1,multi_ref=True)
                result_dic["naive"] = p_value
            
            if method["bonf"]:
                p_value = util.naive_p_mean_bonf(X, ref, output,1,multi_ref=True)
                result_dic["bonf"] = p_value
            
            if method["permutation1"]:
                p_value = util.permutation_test_mean(X, ref, output, model_with_cam,multi_ref=True,test=test)
                result_dic["permutation1"] = p_value
            
            if method["permutation2"]:
                p_value =  util.permutation_test_mean_2(X, ref, output, model_with_cam,multi_ref=True,test=test)
...
if __name__ == "__main__":
    ...
    if is_tracking:
        ...
        EXPERIMENT_NAME = "final_fpr_ltm_mref"
    ...
    result_path = "./experiment_result/fpr_thr_ltm_mref"

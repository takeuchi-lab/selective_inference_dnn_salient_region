...
def run(args):
    ...
    cam_si_thr = cam_si.si4dnn_cam_si_thr(model_with_cam,thr=thr)
    ...
    while flag:
        try:
            ...
            if method["parametric"]:
                p_value = cam_si_thr.inference(X, ref=ref,multi_ref=True,thr=thr)[0]
                result_dic["parametric"] = p_value
            
            if method["oc"]:
                p_value = cam_si_thr.inference(X, ref=ref,oc=True,multi_ref=True,thr=thr)[0]
                result_dic["oc"] = p_value
            
            if method["naive"]:
                p_value =  util.naive_p_mean(X,ref,output,1,multi_ref=True,thr=thr)
                result_dic["naive"] = p_value
            
            if method["bonf"]:
                p_value = util.naive_p_mean_bonf(X, ref, output,1,multi_ref=True,thr=thr)
                result_dic["bonf"] = p_value
            
            if method["permutation1"]:
                p_value = util.permutation_test_mean(X, ref, output, model_with_cam,thr=thr,multi_ref=True,test=test)
                result_dic["permutation1"] = p_value
            
            if method["permutation2"]:
                p_value =  util.permutation_test_mean_2(X, ref, output, model_with_cam,thr=thr,multi_ref=True,test=test)
...
if __name__ == "__main__":
    ...
    if is_tracking:
        ...
        EXPERIMENT_NAME = "final_tpr_ltm_2ref"
    ...
    result_path = "./experiment_result/tpr_thr_ltm_mref"
